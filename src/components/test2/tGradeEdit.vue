    ...
    <div __cid='100' id="pm_1">
        <div id="el-dialog__content"__cid='div'>
            <el-form __cid='224' id="pm_form" ref="editForm" :model="editForm" :rules="rules" :label-width="formLabelWidth">
                                                                    ...
                                                                    <el-cascader filterable clearable change-on-select __cid='216' @change="findTGradePidCascaderDataItemChange" :options="TGradePidOption" v-model="editForm.pid" id="pm_pid">
                                    </el-cascader>
                                                            </el-form-item>
                                                                                                ...
                                                                                                <el-input  __cid='212' v-model.trim="editForm.student_number" placeholder="请输入学生数" id="pm_student_number"></el-input>
                            </el-form-item>
                                                                                                                                                                                                                                                </el-form>
        </div>
...
<script>
    ...
    mounted() {
        this.id = this.rjDialogParams().id;
                                    ...
                                    this.findTGradePidCascaderDataAsync();
                                                                                                        this.findObject();
            },
    methods:{
                                                                                                                                                                go2Submit() {
        ...
        this.$refs["editForm"].validate((valid) => {
            ...
            self.$axios.put("/api/demo/tgrades/"+ self.editForm.id, self.editForm).then((res) => {
                if(res.data.success) {
    ...
    findObject(message){
        this.$axios.get("/api/demo/tgrades/" + this.id).then((res) => {
            if(res.data.success) {
                this.editForm = res.data.result;
                                                                this.editForm.pid = [];
                                                                                                                                        if(message) {
                                                                this.editForm.pid = message.map(v => v.value);
                        this.editForm.pid.unshift("");
                                                                                                                                        }
            }
        }).catch((err) => {
            // 由axios/index.js统一处理
    ...
    doCancel() {
        this.closeRjDialog();
            ...
            findTGradePidCascaderData(){
    this.$axios.get('/api/demo/tgrades/t_grade/cascader/pid_data').then((res) => {
        if(res.data.success){
            var message = res.data.result;
            this.TGradePidOption = message;
            ...
            findTGradePidCascaderDataAsync(){
    this.$axios.get('/api/demo/tgrades/t_grade/cascader/pid_data/parent?id='+this.id).then(async (res) => {
            if(res.data.success){
            var message = res.data.result;
            if (message) {
    ...
    findTGradePidCascaderDataAwait(pid){
    this.$axios.get('/api/demo/tgrades/t_grade/cascader/pid_data?pid='+pid).then((res) => {
            if(res.data.success){
            var message = res.data.result;
            if(message != undefined && message.length > 0) {
                setChildrenByCurrentId(this.TGradePidOption,pid,message);
            }
        }
    }).catch((err) => {
    });
