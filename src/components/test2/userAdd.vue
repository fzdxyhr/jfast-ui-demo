		...
		<div id="el-dialog__content" __cid='div'>
			<el-form __cid='224' id="pm_form" ref="addForm" :model="addForm" :label-width="formLabelWidth" :rules="rules">
				<el-form-item __cid='225' prop="name" label="名字" id="pm_name_form_item">
					<el-input __cid='212' v-model.trim="addForm.name" placeholder="请输入名字" id="pm_name"></el-input>
				</el-form-item>
				<el-form-item __cid='225' prop="sex" label="性别" id="pm_sex_form_item">
					<el-select __cid='214' clearable v-model="addForm.sex" placeholder="请选择性别" id="pm_sex" __model='{"options":[{"label":"男","value":1},{"label":"女","value":2}]}'>
						<el-option key="1" label="男" :value="1"></el-option>
						<el-option key="2" label="女" :value="2"></el-option>
					</el-select>
				</el-form-item>
				<el-form-item __cid='225' prop="age" label="年龄" id="pm_age_form_item">
					<el-select __cid='214' clearable v-model="addForm.age" id="pm_age" placeholder="请选择年龄" __model='{"options":[],"innerHTML":"<el-option v-for=\"item in TGradeOption3\" :key=\"item.value\" :label=\"item.label\" :value=\"item.value\"></el-option>"}'>
				...
				<el-form-item __cid='225' prop="class_name" label="班级名称" id="pm_class_name_form_item">
					<el-input __cid='212' v-model.trim="addForm.class_name" placeholder="请输入班级名称" id="pm_class_name"></el-input>
				</el-form-item>
			</el-form>
		</div>
			...
			go2Submit() {
				this.$refs["addForm"].validate((valid) => {
					if (valid) {
						...
						this.$axios.post("/api/demo/users", this.addForm).then((res) => {
							if (res.data.success) {
								this.$message({
									type: 'success',
									message: '添加成功!'
